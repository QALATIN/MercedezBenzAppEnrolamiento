...
class NfcFragment : Fragment(), NfcTagReadingListener {
    ...
    private val TAG = "NfcFragment.class"
        ...
        try {
            if (nfcAdapter != null) {
                ...
                BinnacleCongif.writeLog(
                    TAG,
                    1,
                    "Error NFC is not available for this device -> function: nfcPressed()",
                    "Error: NFC",
                    requireContext()
                )
    ...
    override fun onError(acuantError: AcuantError) {
        BinnacleCongif.writeLog(
            TAG,
            1,
            "Error NFC -> function: onError()",
            "Error: "+acuantError.errorDescription,
            requireContext()
        )
