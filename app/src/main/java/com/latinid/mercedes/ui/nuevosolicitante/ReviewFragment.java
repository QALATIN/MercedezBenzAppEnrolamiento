...
public class ReviewFragment extends Fragment {
        ...
        binding.siguiente.setOnClickListener(view -> {
            ...
            executor.execute(() -> {
               ...
               /* requireActivity().runOnUiThread(() -> {
                    ...
                    ((Main3Activity) getActivity()).replaceFragments(SendInfoFragment.class);
                });*/
                switch (DatosRecolectados.docSignatureFinish){
                    ...
                    case  "nosigned":
                        requireActivity().runOnUiThread(() -> {
                            ...
                            message.show(getParentFragmentManager(), "Alerta");
                            ((Main3Activity) getActivity()).replaceFragments(InProcessFragment.class);
                        });
                        break;
                }
            });
    ...
    private void hideKeyBoard() {
        ...
        binding.outlinedNacimiento.getEditText().setOnClickListener(view -> {
            try {
                ...
                BinnacleCongif.writeLog(TAG,1, "Error buscando fecha -> function: hideKeyBoard()", "Error: "+e.getLocalizedMessage(), requireContext());
                Log.e("TAG", "ERROR()", e);
            }
        });
    ...
    private void fillFields() {
        ...
        try {
            ...
            BinnacleCongif.writeLog(TAG,1, "Error pintando nombre -> function: fillFields()", "Error: "+e.getLocalizedMessage(), requireContext());
            e.printStackTrace();
        }
        try {
            binding.outlinedFather.getEditText().setText(persona.getPaterno().replaceAll("[^a-zA-ZñÑ0-9]", " ").trim());
        } catch (Throwable e) {
            BinnacleCongif.writeLog(TAG,1, "Error pintando apellido paterno -> function: fillFields()", "Error: "+e.getLocalizedMessage(), requireContext());
            e.printStackTrace();
        }
        try {
            ...
            BinnacleCongif.writeLog(TAG,1, "Error pintando apellido materno -> function: fillFields()", "Error: "+e.getLocalizedMessage(), requireContext());
            e.printStackTrace();
        }
        try {
            binding.outlinedCurp.getEditText().setText(persona.getCurp().replaceAll("[^a-zA-Z0-9]", "").trim());
        } catch (Throwable e) {
            BinnacleCongif.writeLog(TAG,1, "Error pintando curp -> function: fillFields()", "Error: "+e.getLocalizedMessage(), requireContext());
            e.printStackTrace();
        }
        try {
            ...
            BinnacleCongif.writeLog(TAG,1, "Error pintando nacimiento -> function: fillFields()", "Error: "+e.getLocalizedMessage(), requireContext());
            if(binding.outlinedCurp.getEditText().getText().length() != 0){
                String sub1 = binding.outlinedCurp.getEditText().getText().toString().substring(4, 10);
                String a = sub1.substring(0, 2);
                String mm = sub1.substring(2, 4);
        ...
        try {
            ...
            BinnacleCongif.writeLog(TAG,1, "Error pintando domicilio -> function: fillFields()", "Error: "+e.getLocalizedMessage(), requireContext());
            e.printStackTrace();
        }
        try {
            binding.outlinedGender.getEditText().setText((persona.getSexo().replaceAll("[^a-zA-Z0-9]", "").trim().equals("M")) ? "Masculino" : "Femenino");
        } catch (Throwable e) {
            BinnacleCongif.writeLog(TAG,1, "Error pintando genero -> function: fillFields()", "Error: "+e.getLocalizedMessage(), requireContext());
            e.printStackTrace();
        }
        try {
            ...
            BinnacleCongif.writeLog(TAG,1, "Error pintando edad -> function: fillFields()", "Error: "+e.getLocalizedMessage(), requireContext());
            e.printStackTrace();
        }
        try {
            if(binding.outlinedAge.getEditText().getText().length() == 2){
                ...
                try {
                    ...
                    BinnacleCongif.writeLog(TAG,1, "Error pintando edad -> function: fillFields()", "Error: "+e2.getMessage()+"|"+e2.getCause().getMessage(), requireContext());
                    e2.printStackTrace();
                    try {
