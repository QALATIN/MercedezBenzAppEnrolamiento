...
public class CompleteProcessFragment extends Fragment {
    ...
    private static final String TAG = "CompleteProcessFragment.class";
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding.buttonBack.setOnClickListener(view -> {
            ((Main3Activity) getActivity()).removerFragmets();
        });
        executor.execute(() -> {
            ...
            try {
                ...
                BinnacleCongif.writeLog(TAG,1, "Error seleccionando fechas -> function: onCreateView()", "Error: "+e.getLocalizedMessage(), requireContext());
                e.printStackTrace();
            }
    ...
    private void loadCalendars(long one, long two) {
        ...
        for (ActiveEnrollment activeEnrollment : activeEnrollments) {
            try {
                ...
                BinnacleCongif.writeLog(TAG,1, "Error filandro enrrolamientos -> function: loadCalendars()", "Error: "+e.getLocalizedMessage(), requireContext());
                e.printStackTrace();
            }
        }
    }
            ...
            for (ActiveEnrollment activeEnrollment : activeEnrollmentsTemp) {
                ...
                if (activeEnrollment.getState_id().equals("8") || activeEnrollment.getState_id().equals("9")) {
                    ...
                    try {
                        ...
                        BinnacleCongif.writeLog(TAG,1, "ErrorControlate -> function: createViews()", "Error: "+e.getLocalizedMessage(), requireContext());
            ...
            try {
                if (enrollment.getEnroll_solicitante().equals(activeEnrollment.getEnroll_id()) && enrollment.getTipo_enroll().equals("Aval")) {
                    try {
                        ...
                        BinnacleCongif.writeLog(TAG,1, "Error revisando avales-> function: createSubViews()", "Error: "+e.getLocalizedMessage(), requireContext());
                ...
                if (enrollment.getEnroll_solicitante().equals(activeEnrollment.getEnroll_id()) && enrollment.getTipo_enroll().equals("Coacreditado")) {
                    try {
                        ...
                        BinnacleCongif.writeLog(TAG,1, "Error revisando coacreditados -> function: createSubViews()", "Error: "+e.getLocalizedMessage(), requireContext());
                        Log.e(TAG, "Error,", e);
                ...
                BinnacleCongif.writeLog(TAG,1, "Error revisando avales y coacreditados -> function: createSubViews()", "Error: "+e.getLocalizedMessage(), requireContext());
            }
        }
    ...
    private void completeEnrollment(ActiveEnrollment activeEnrollment) {
        executor.execute(() -> {
            ...
            switch (activeEnrollment.getState_id()) {
                case "1":
                    requireActivity().runOnUiThread(() -> {
                        ((Main3Activity) getActivity()).replaceFragments(IdentificacionFragment.class);
                    });
                    break;
                case "2":
                    requireActivity().runOnUiThread(() -> {
                        ((Main3Activity) getActivity()).replaceFragments(FaceAwareFragment.class);
                    });
                    break;
                case "3":
                    requireActivity().runOnUiThread(() -> {
                ...
                case "5":
                    requireActivity().runOnUiThread(() -> {
                        ((Main3Activity) getActivity()).replaceFragments(ReviewFragment.class);
                    });
                    break;
                case "7":
